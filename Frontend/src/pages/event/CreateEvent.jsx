...
function CreateEvent() {
  ...
  const addNestedItem = (section) => {
    const emptyItem = section === 'prizes' ?
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-indigo-50 to-pink-50 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          {/* LEFT: Basic + Registration - ENHANCED */}
          <div className="bg-white/10 backdrop-blur-3xl shadow-2xl rounded-3xl p-10 border border-white/20 hover:shadow-3xl hover:-translate-y-2 transition-all duration-500">
            ...
            {/* Title - PREMIUM */}
            <div className="mb-8">
              <label className="block text-lg font-bold text-gray-700 mb-4 tracking-wide">Event Title *</label>
              <input 
                name="title" 
                ...
                placeholder="Enter captivating event title..." required 
              />
            </div>
            
            {/* Description - PREMIUM */}
            <div className="mb-10">
              <label className="block text-lg font-bold text-gray-700 mb-4 tracking-wide">Description *</label>
              <textarea 
                name="description" 
                ...
                placeholder="Share the excitement of your event..." required 
              />
            </div>

            {/* Category + College ID - 2x2 PREMIUM */}
            <div className="grid grid-cols-2 gap-8 mb-10">
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-4">College ID</label>
                <input name="college_id" value={formData.college_id} onChange={handleInputChange}
                  className="w-full px-6 py-5 bg-white/80 border-2 border-gray-200/50 rounded-2xl 
            ...
            {/* Location + Venue + Organizer - PREMIUM */}
            <div className="grid grid-cols-2 gap-8 mb-10">
              <div>
              ...
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-4">Venue *</label>
                <input name="venue" value={formData.venue} onChange={handleInputChange}
                  className="w-full px-6 py-5 bg-white/80 border-2 border-gray-200/50 rounded-2xl 
                            focus:ring-4 focus:ring-orange-400/60 focus:border-orange-400 shadow-xl 
                            hover:shadow-2xl hover:border-orange-300/70 transition-all duration-500" required />
              </div>
            </div>

            <div className="mb-10">
              <label className="block text-sm font-bold text-gray-700 mb-4">Organizer *</label>
              <input name="organizer" value={formData.organizer} onChange={handleInputChange}
                className="w-full px-6 py-5 bg-white/80 border-2 border-gray-200/50 rounded-2xl 
                          ...
                          hover:shadow-2xl transition-all duration-500" required />
            </div>

            {/* Dates & Registration - PREMIUM */}
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 p-10 rounded-3xl border-4 border-emerald-200/50 mb-10">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block font-bold text-gray-700 mb-3">Start Date *</label>
                  <input type="date" name="start_date" value={formData.start_date} onChange={handleInputChange}
                    className="w-full px-6 py-5 bg-white/90 border-2 border-emerald-200/50 rounded-2xl 
                              shadow-lg hover:shadow-xl transition-all duration-300" required />
                </div>
                <div>
                  <label className="block font-bold text-gray-700 mb-3">End Date *</label>
                  <input type="date" name="end_date" value={formData.end_date} onChange={handleInputChange}
                    className="w-full px-6 py-5 bg-white/90 border-2 border-emerald-200/50 rounded-2xl 
                              shadow-lg hover:shadow-xl transition-all duration-300" required />
                </div>
                <div>
                  <label className="block font-bold text-gray-700 mb-3">Registration Start</label>
                  <input type="date" name="registration_start" value={formData.registration_start} onChange={handleInputChange}
                    className="w-full px-6 py-5 bg-white/90 border-2 border-emerald-200/50 rounded-2xl shadow-lg" />
                </div>
                <div>
                  <label className="block font-bold text-gray-700 mb-3">Registration End</label>
                  <input type="date" name="registration_end" value={formData.registration_end} onChange={handleInputChange}
                    className="w-full px-6 py-5 bg-white/90 border-2 border-emerald-200/50 rounded-2xl shadow-lg" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6 mt-8 pt-8 border-t-2 border-emerald-200">
                <div>
                  <label className="block font-bold text-gray-700 mb-3">Max Participants</label>
                  <input type="number" name="max_participants" value={formData.max_participants} 
                    onChange={handleInputChange} className="w-full px-6 py-5 bg-white/90 border-2 
                    border-purple-200/50 rounded-2xl shadow-lg hover:shadow-xl transition-all" min="1" />
                </div>
                <div>
                  <label className="block font-bold text-gray-700 mb-3">Registration Fee ₹</label>
                  <input type="number" name="registration_fee" value={formData.registration_fee} 
                    onChange={handleInputChange} className="w-full px-6 py-5 bg-white/90 border-2 
                    border-amber-200/50 rounded-2xl shadow-lg hover:shadow-xl transition-all" step="0.01" />
                </div>
              </div>
            </div>
          </div>

          {/* RIGHT: Advanced - ENHANCED */}
          <div className="space-y-8">
            {/* Media Upload - PREMIUM */}
            <div className="bg-white/10 backdrop-blur-3xl shadow-2xl rounded-3xl p-10 border border-white/20 hover:shadow-3xl transition-all">
              ...
              <div className="mb-8">
                <label className="block text-lg font-bold text-gray-700 mb-4">Event Banner Image</label>
                <input type="file" name="image_url" accept="image/*" onChange={handleInputChange}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block font-bold text-gray-700 mb-3">Tags</label>
                ...
                <div className="flex items-center justify-center p-6 bg-gradient-to-r from-emerald-50 to-teal-50 
                  ...
                  <input type="checkbox" name="is_featured" checked={formData.is_featured} 
                    onChange={handleInputChange} className="w-8 h-8 text-emerald-600 rounded-xl mr-4 
                    focus:ring-emerald-500 border-2 border-emerald-400 shadow-lg transform hover:scale-110" />
                  <span className="text-xl font-bold text-emerald-800">⭐ Featured Event</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-3xl shadow-2xl rounded-3xl p-8 border border-white/20">
              <h3 className="text-xl font-bold bg-gradient-to-r from-amber-600 to-orange-600 bg-clip-text text-transparent 
                ...
                🏆 Prizes
                <button type="button" onClick={() => addNestedItem('prizes')} 
              ...
              {formData.prizes.map((prize, index) => (
                <div key={index} className="bg-white/60 backdrop-blur-xl p-6 rounded-2xl mb-4 shadow-xl hover:shadow-2xl border border-white/50 transition-all">
                  <div className="grid grid-cols-3 gap-4">
                    <input placeholder="1st" value={prize.position} 
                      onChange={(e) => handleNestedChange('prizes', index, 'position', e.target.value)}
                    ...
                    <input placeholder="Trophy" value={prize.prize} 
                      onChange={(e) => handleNestedChange('prizes', index, 'prize', e.target.value)}
                      className="px-4 py-3 border-2 border-gray-200/50 rounded-xl bg-white/80 shadow-md 
                                 focus:ring-2 focus:ring-yellow-400 hover:shadow-lg transition-all text-sm" />
                    <input type="number" placeholder="₹5000" value={prize.amount} 
                      onChange={(e) => handleNestedChange('prizes', index, 'amount', e.target.value)}
                      className="px-4 py-3 border-2 border-gray-200/50 rounded-xl bg-white/80 shadow-md 
                                 focus:ring-2 focus:ring-yellow-400 hover:shadow-lg transition-all text-sm" />
                  </div>
                  <button type="button" onClick={() => removeNestedItem('prizes', index)}
            ...
            <button type="submit" className="w-full col-span-2 lg:col-span-1 bg-gradient-to-r 
